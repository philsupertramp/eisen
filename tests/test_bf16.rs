...
fn test_bf16_kernel_roundtrip() {
    ...
    let f_cast = g
        .functions
        .get("cast_f32_to_bf16")
        .expect("BF16 cast kernel missing. Check build.rs sm_89 flag.");
    ...
    let f_reconstruct = g
        .functions
        .get("cast_bf16_to_f32")
        .expect("BF16 reconstruct kernel missing.");
    ...
    for (original, reconstructed) in input_data.iter().zip(reconstructed_data.iter()) {
        assert!(
            ...
            "BF16 precision loss exceeded threshold or kernel failed."
        );
    }
}
...
#[test]
fn test_force_fp32_env_disables_bf16_mixed_mode() {
    let device = match setup_gpu() {
        Some(d) => d,
        None => {
            eprintln!("No GPU found, skipping BF16 precision-policy test.");
            return;
        }
    };

    unsafe {
        std::env::set_var("EISEN_FORCE_FP32", "1");
    }
    let g = Graph::new(device);
    assert!(
        !g.uses_bf16_mixed_precision(),
        "EISEN_FORCE_FP32 must disable BF16 mixed precision"
    );
    unsafe {
        std::env::remove_var("EISEN_FORCE_FP32");
    }
}
