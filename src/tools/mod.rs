...
use std::io::{Write, BufWriter, Read, Seek, SeekFrom, BufRead, BufReader};
use std::path::Path;
use std::sync::Arc;
...
pub fn train_on_sampled_data(
    ...
    println!("Sampling {}MB from {}GB file...", sample_size_mb, file_size / (1024*1024*1024));
    ...
    tokenizer.save(output_tokenizer_path).expect("Failed to save tokenizer");
    println!("Success!");
}
...
pub fn pre_tokenize_resumable(txt_input_path: &str, bin_output_path: &str, state_path: &str, tokenizer: Arc<BPETokenizer>) {
    ...
    if Path::new(state_path).exists() {
        ...
        if let Ok(line) = state_str.trim().parse::<usize>() {
            start_line = line;
            println!("Recovered state! Resuming pre-tokenization from line {}...", start_line);
        }
    }
    ...
    for (i, line) in reader.lines().enumerate() {
        if i < start_line { continue; } // Skip lines we already processed
        ...
        if i % 10000 == 0 && i > start_line {
            ...
            println!("Processed {} lines... (Appended {} tokens this session)", i, tokens_written);
        }
    }
