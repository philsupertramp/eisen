...
impl Attention {
    ...
    pub fn forward_with_mask(&self, g: &mut Graph, x_id: usize, causal: bool) -> usize {
        ...
        assert_eq!(shape.len(), 3, "Attention requires 3D input [Batch, Seq, Dim]");
        let batch = shape[0];
        let seq_len = shape[1];
        ...
        let masked_scores_id = if causal {
            ...
            for r in 0..seq_len {
                for c in 0..seq_len {
                    ...
                    if c > r { mask_data[r * seq_len + c] = -1e20; }
                }
            }
        ...
        let context_id = g.bmm(probs_id, v_id, false);
...
impl MultiHeadAttention {
    pub fn new(g: &mut Graph, hidden_dim: usize, num_heads: usize) -> Self {
        assert!(hidden_dim % num_heads == 0, "hidden_dim must be cleanly divisible by num_heads");
    ...
    pub fn forward_with_mask(&self, g: &mut Graph, x_id: usize, causal: bool) -> usize {
        ...
        let q_4d = g.reshape(q_rope_id, vec![batch, seq_len, self.num_heads, self.head_dim]);
        let k_4d = g.reshape(k_rope_id, vec![batch, seq_len, self.num_heads, self.head_dim]);
        ...
        let scale_id = g.alloc(vec![bh, seq_len, seq_len], vec![scale; bh * seq_len * seq_len]);
        let scaled_scores_id = g.mul(scores_id, scale_id);
        ...
        let context_flat = g.bmm(probs_id, v_flat, false);
        
        // 10. Reshape -> [Batch, Heads, Seq, HeadDim]
        let context_4d = g.reshape(context_flat, vec![batch, self.num_heads, seq_len, self.head_dim]);
