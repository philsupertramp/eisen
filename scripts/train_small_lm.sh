...
export EISEN_FIM_RATE="${EISEN_FIM_RATE:-0.0}"
export EISEN_FIM_SPM_RATE="${EISEN_FIM_SPM_RATE:-0.0}"

export EISEN_VRAM_BUDGET_MB="${EISEN_VRAM_BUDGET_MB:-6000}"
export EISEN_ACTIVATION_RESERVE_MB="${EISEN_ACTIVATION_RESERVE_MB:-1500}"

export EISEN_PRECISION="${EISEN_PRECISION:-''}"
EISEN_DTYPE=""

if [ "${EISEN_PRECISION}" = "bf16" ]; then
...
cargo build --release  --example train_llm ${EISEN_DTYPE}
#
./target/release/examples/train_llm ${EISEN_DTYPE}
